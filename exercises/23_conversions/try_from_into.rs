// `TryFrom` 提供简单、安全的类型转换，允许在某些情况下以可控的方式失败。
// 它与 `From` 基本相同，主要区别在于：
// 它返回的是 `Result` 类型，而不是目标类型本身。
// 更多信息请参阅文档：
...
fn main() {
    ...
    let v = vec![183, 65, 14];
    // 对于切片，可以使用 `try_from` 函数，
    let c3 = Color::try_from(&v[..]);
    println!("{c3:?}");
    // 或者用圆括号包裹切片表达式，再调用 `try_into`。
